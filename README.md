...
|         提出日         |       `\submitdate{YYYYMMDD}`        |                                              西暦8ケタで入力                                               | 未入力の場合、自動でコンパイル日が入力されます。1ケタ台の月日な場合、自動で十の位の0が抜けます。                                                                                                                                                   |
|        再提出日        |      `\resubmitdate{YYYYMMDD}`       |                                              西暦8ケタで入力                                               |                                                                                                                                                                                                                                                    |
|       実験実施日       |    `\exdates{YYYYMMDD/温度/湿度}`    | 西暦8ケタ, 温度, 湿度のセットをスラッシュ区切りにし、複数回分をカンマ区切りで指定する。最大8回分まで対応。 | 入力したケタ数で表示されます。有効数字の補正はありません。温度・湿度が未入力の場合は警告が出ますが、コンパイルは通ります(意図的な未計測は `YYYYMMDD//湿度` のように空スラッシュで指定可能)。日付形式が不正な場合はその回のみ無視され警告が出ます。 |
...
### 実験題目名が長くても自動でフォントサイズを調整

実験題目名が長い場合、指定の枠から文字がはみ出さないよう、自動的にフォントサイズが縮小されます。
文字数に応じて3段階 (通常・14pt・11pt) に切り替わり、LuaLaTeX・upLaTeX・XeLaTeXの3エンジンで常に同じ表示になります。
...
`\submitdate{}` に何も値を渡さなかった場合、自動でコンパイル日が入力されます。
わざわざ日付を入力する手間を省けます。

### 入力ミスを自動で検知

`\exdates` の温度・湿度が抜けている場合や、日付・数値の形式が誤っている場合に、コンパイル時の警告として通知されます。
温度・湿度を意図的に空欄にしたい場合（`YYYYMMDD//湿度` のようにスラッシュだけ残す形）はコンパイルが問題なく通るため、未計測の回にも対応できます。
